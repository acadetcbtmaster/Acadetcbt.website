    ...
    <div className="space-y-6" id="audit-compliance-module-root">
      ...
      <div className="bg-slate-900 border border-slate-800 p-5 rounded-2xl flex flex-col md:flex-row md:items-center justify-between gap-4 shadow-xl">
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          >
            <RefreshCw className={`w-3.5 h-3.5 ${isLiveUpdating ? 'animate-spin text-blue-400' : ''}`} />
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-3 sm:gap-4">
        ...
        >
          ...
          <p className="text-2xl font-black text-blue-400 mt-1">{permissionChanges}</p>
          <span className="text-[10px] text-blue-400/80 font-medium mt-1 block">Role & Matrix Modifications</span>
        </div>
        ...
        >
          ...
          <p className="text-2xl font-black text-blue-300 mt-1">{financialEvents}</p>
          <span className="text-[10px] text-blue-400 font-medium mt-1 block">Payment Approvals & Refunds</span>
        </div>
      ...
      {activeTab === 'compliance_flags' && (
        <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 space-y-6">
          <div className="flex items-center justify-between pb-4 border-b border-slate-800">
            ...
            <span className="px-3 py-1 bg-blue-500/10 text-blue-300 border border-blue-500/30 text-xs font-bold rounded-full">
              ...
              <div key={idx} className="bg-slate-950 border border-slate-800 p-4 rounded-xl flex items-start justify-between gap-3">
                <div className="space-y-1">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="px-2 py-0.5 bg-blue-500/10 text-blue-400 font-bold rounded text-[10px]">{guard.status}</span>
                  </div>
                ...
                >
                  <div className="flex justify-between items-start gap-2">
                    ...
                    <span className={`px-2 py-0.5 rounded text-[10px] font-bold ${
                      inv.status === 'Open' ? 'bg-amber-500/20 text-amber-300' : inv.status === 'Under Review' ? 'bg-sky-500/20 text-sky-300' : 'bg-blue-500/20 text-blue-300'
                    }`}>
                      {inv.status}
                    </span>
                  </div>
      ...
      {activeTab === 'risk_assessment' && (
        <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 space-y-6">
          <div className="flex items-center justify-between pb-4 border-b border-slate-800">
            ...
            <span className="px-3.5 py-1 bg-blue-500/10 text-blue-400 border border-blue-500/30 text-xs font-black rounded-full">
              ...
              <div key={risk.id} className="bg-slate-950 border border-slate-800 p-5 rounded-xl space-y-3">
                <div className="flex items-center justify-between">
                  ...
                  <span className={`px-2 py-0.5 rounded font-extrabold text-[10px] ${
                    risk.level.includes('Low') ? 'bg-blue-500/20 text-blue-300' : 'bg-amber-500/20 text-amber-300'
                  }`}>
