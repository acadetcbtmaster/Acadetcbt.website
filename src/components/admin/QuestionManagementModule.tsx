...
export const QuestionManagementModule: React.FC<QuestionManagementModuleProps> = ({
  ...
  const isAuthorized = hasPermission(currentAdmin, 'manage_questions');

  if (!isAuthorized) {
    return (
      <div className="bg-slate-900 border border-rose-500/40 p-8 sm:p-12 rounded-2xl shadow-2xl space-y-6 text-center max-w-xl mx-auto my-12" id="questions-access-restricted">
        ...
        <div className="space-y-2">
          ...
          <p className="text-sm text-slate-300 max-w-md mx-auto leading-relaxed">
            You do not have permission to access this section. Question bank management is strictly restricted to authorized roles.
          </p>
        </div>
      </div>
    );
    ...
    <div className="space-y-6 animate-in fade-in duration-300">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-3">
        ...
        <div
          ...
          className="bg-slate-900 border border-slate-800/80 p-4 rounded-2xl cursor-pointer hover:border-blue-500/50 transition-all group"
        >
          <div className="flex justify-between items-center text-slate-400 mb-1">
            <span className="text-[11px] font-bold uppercase tracking-wider text-blue-400">Published</span>
            <CheckCircle2 className="w-4 h-4 text-blue-400 group-hover:scale-110 transition-transform" />
          </div>
          <p className="text-2xl font-black text-blue-400">{summary.published.toLocaleString()}</p>
          <span className="text-[10px] text-blue-500/80 mt-1 block">Active in CBT Sessions</span>
        </div>
              ...
              <div className="flex flex-wrap items-center gap-2 w-full md:w-auto">
                ...
                <div className="flex flex-wrap items-center gap-1.5 ml-auto">
                  <button
                    ...
                    className="px-3.5 py-2 bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-500 hover:to-teal-500 text-white font-extrabold text-xs rounded-xl flex items-center gap-1.5 cursor-pointer shadow-md transition-all"
                  ...
                  <button
                    ...
                    className="px-3 py-2 bg-blue-950/60 hover:bg-blue-900 border border-blue-500/30 text-blue-300 font-bold text-xs rounded-xl flex items-center gap-1.5 cursor-pointer"
                    title="Export CSV"
                  >
              ...
              <div className="flex flex-wrap items-center justify-between gap-3">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <button
                    ...
                    className="px-4 py-2 bg-blue-600 hover:bg-blue-500 text-white font-extrabold text-xs rounded-xl shadow-lg cursor-pointer flex items-center gap-2 transition-colors"
                  >
              ...
              <div className="space-y-3">
                {extractedQuestions.map((eq, i) => (
                  <div key={eq.id} className="p-4 bg-slate-950 border border-slate-800 rounded-xl space-y-2 relative group">
                    ...
                    <p className="text-[11px] text-blue-400 font-bold">Correct Answer: Option {eq.correctAnswer}</p>
                  </div>
                ))}
              </div>
            </div>
                ...
                <div key={pq.id} className="bg-slate-900 border border-slate-800 p-6 rounded-2xl space-y-4 shadow-xl">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div className="p-4 bg-slate-950 border border-slate-800 rounded-xl space-y-2">
                      ...
                      <span className="text-[10px] font-bold text-blue-400 block">Correct Option: {pq.correctAnswer}</span>
                    </div>
                    ...
                    <div className="p-4 bg-indigo-950/30 border border-indigo-500/30 rounded-xl space-y-2">
                      <span className="text-[10px] font-bold text-indigo-400 uppercase tracking-wider block flex items-center justify-between">
                        ...
                        <span className="text-blue-400">Score: {pq.qualityScore || '94/100'}</span>
                      </span>
                  ...
                  <div className="flex flex-wrap justify-end gap-2 pt-2 border-t border-slate-800">
                    ...
                    {pq.status === 'Publishing Queue' && (
                      <button
                        ...
                        className="px-4 py-2 bg-blue-500 hover:bg-blue-400 text-slate-950 text-xs font-black rounded-xl cursor-pointer shadow-lg"
                      >
                  ...
                  return (
                    <div key={lvl} className="space-y-1">
                      ...
                      <div className="w-full h-2 bg-slate-950 rounded-full overflow-hidden">
                        <div
                          className={`h-full ${
                            lvl === 'Easy' ? 'bg-blue-500' : lvl === 'Medium' ? 'bg-amber-500' : 'bg-rose-500'
                          }`}
                          style={{ width: `${pct}%` }}
                        />
                      </div>
                ...
                {qDiagramUrl ? (
                  <div className="p-3 bg-slate-950 border border-slate-800 rounded-xl space-y-2">
                    ...
                    <div className="flex justify-between items-center text-xs">
                      <span className="text-blue-400 font-bold">Diagram Attached</span>
                      <button
                        type="button"
                        onClick={() => {
                          setQDiagramUrl('');
          ...
          <div className="bg-slate-900 border border-slate-800 w-full max-w-xl rounded-2xl p-6 space-y-4 max-h-[90vh] overflow-y-auto custom-scrollbar shadow-2xl animate-in zoom-in-95">
            ...
            <div className="space-y-3 text-xs">
              ...
              <div className="grid grid-cols-2 gap-2 font-mono">
                <div className={`p-2.5 rounded-lg border ${viewingQuestion.correctAnswer === 'A' ? 'bg-blue-950/40 border-blue-500/50 text-blue-200' : 'bg-slate-950 border-slate-800 text-slate-300'}`}>
                  A. {viewingQuestion.optionA}
                </div>
                <div className={`p-2.5 rounded-lg border ${viewingQuestion.correctAnswer === 'B' ? 'bg-blue-950/40 border-blue-500/50 text-blue-200' : 'bg-slate-950 border-slate-800 text-slate-300'}`}>
                  B. {viewingQuestion.optionB}
                </div>
                <div className={`p-2.5 rounded-lg border ${viewingQuestion.correctAnswer === 'C' ? 'bg-blue-950/40 border-blue-500/50 text-blue-200' : 'bg-slate-950 border-slate-800 text-slate-300'}`}>
                  C. {viewingQuestion.optionC}
                </div>
                <div className={`p-2.5 rounded-lg border ${viewingQuestion.correctAnswer === 'D' ? 'bg-blue-950/40 border-blue-500/50 text-blue-200' : 'bg-slate-950 border-slate-800 text-slate-300'}`}>
