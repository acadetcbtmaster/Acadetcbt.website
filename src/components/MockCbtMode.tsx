...
export const MockCbtMode: React.FC<MockCbtModeProps> = ({
  ...
  const handleStartExam = () => {
    ...
    const shuffledExamQuestions = shuffleArray(selected);
  ...
  return (
    <div className="w-full max-w-6xl mx-auto px-4 py-8 space-y-6" id="mock-cbt-container">
      ...
      {step === 'config' && (
        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-5 sm:p-7 shadow-xs space-y-5 max-w-3xl mx-auto">
          <div className="flex items-center space-x-3 pb-4 border-b border-slate-200 dark:border-slate-800">
            <div className="w-11 h-11 rounded-xl bg-blue-50 dark:bg-blue-950/30 text-[#1e3a8a] dark:text-blue-400 border border-blue-200 dark:border-blue-500/30 flex items-center justify-center">
              <BookOpen className="w-5 h-5" />
            </div>
            <div>
              <h1 className="text-xl sm:text-2xl font-bold text-slate-900 dark:text-white">Mock CBT Exam Setup</h1>
              <p className="text-xs text-slate-500 dark:text-slate-400">Simulate official computer-based examination conditions.</p>
            </div>
          </div>

          {/* Academic Hierarchy 6-Step Cascading Flow */}
          <div className="bg-slate-50 dark:bg-slate-950/60 p-4 rounded-xl border border-slate-200 dark:border-slate-800/80">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              <label className="block text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1.5">Exam Duration (Editable)</label>
              <div className="space-y-2">
                <div className="flex items-center gap-2">
                  <input
                    ...
                    className="w-24 p-2.5 bg-white dark:bg-slate-950 border border-slate-200 dark:border-slate-700 rounded-xl text-xs font-bold text-slate-900 dark:text-white text-center focus:border-[#1e3a8a] focus:outline-none"
                    placeholder="Minutes"
                  />
                  <span className="text-xs text-slate-600 dark:text-slate-300 font-semibold">Minutes</span>
                </div>
            ...
            <div>
              <label className="block text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1.5">Number of Questions</label>
              <select
                value={questionCount}
                onChange={(e) => {
                ...
                className="w-full p-2.5 bg-white dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-xl text-xs text-slate-800 dark:text-slate-200 focus:border-[#1e3a8a] font-medium cursor-pointer"
              >
                ...
                <option value="unlimited" className="font-bold text-amber-500">
          ...
          <button
            ...
            className="w-full py-3.5 bg-[#1e3a8a] hover:bg-[#172554] text-white font-bold text-xs sm:text-sm rounded-xl shadow-xs transition-colors flex items-center justify-center gap-2 cursor-pointer"
        ...
        <div className="space-y-6" id="active-cbt-screen">
          ...
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl p-4 shadow-xs flex flex-wrap items-center justify-between gap-4">
            ...
            <div className="flex items-center space-x-3">
              <div className="w-10 h-10 rounded-xl bg-blue-50 dark:bg-blue-950/30 text-[#1e3a8a] dark:text-blue-400 border border-blue-200 dark:border-blue-500/30 flex items-center justify-center font-bold text-sm">
                {user.name.charAt(0)}
              </div>
              <div>
                <p className="text-xs font-bold text-slate-900 dark:text-white">{user.name}</p>
                <p className="text-[10px] text-slate-500 dark:text-slate-400 uppercase">Reg No: CBT/2026/{user.id.slice(-5)}</p>
              </div>
            </div>

            {/* Course Code Badge */}
            <div className="hidden sm:block text-center">
              <span className="text-xs font-bold text-[#1e3a8a] dark:text-blue-400 bg-blue-50 dark:bg-blue-500/10 px-3 py-1 rounded-lg border border-blue-200 dark:border-blue-500/20">
                {courses.find((c) => c.id === selectedCourseId)?.code || 'GST101'}
              </span>
            </div>

            {/* Countdown Timer Badge */}
            <div className={`px-4 py-2 rounded-xl border flex items-center gap-2 text-sm font-black font-mono shadow-xs ${
              isTimeLow
                ? 'bg-rose-50 dark:bg-rose-500/20 text-rose-600 dark:text-rose-300 border-rose-200 dark:border-rose-500/50 animate-pulse'
                : 'bg-blue-50 dark:bg-slate-950 text-[#1e3a8a] dark:text-blue-400 border-blue-200 dark:border-slate-800'
            }`}>
            ...
            <button
              ...
              className="p-2.5 bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-300 rounded-xl border border-slate-200 dark:border-slate-700 transition-colors flex items-center gap-1.5 text-xs font-semibold cursor-pointer shadow-xs"
              id="cbt-calculator-toggle-btn"
            >
              <Calculator className="w-4 h-4 text-[#1e3a8a] dark:text-blue-400" />
          ...
          {calculatorOpen && (
            <div className="fixed bottom-6 right-6 z-50 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 p-4 rounded-2xl shadow-xl w-64 animate-in slide-in-from-bottom-3">
              <div className="flex justify-between items-center pb-2 mb-2 border-b border-slate-200 dark:border-slate-800">
                <span className="text-xs font-bold text-slate-900 dark:text-slate-300">CBT Scientific Calculator</span>
                <button onClick={() => setCalculatorOpen(false)} className="text-slate-400 hover:text-slate-900 dark:hover:text-white cursor-pointer">
                  <X className="w-4 h-4" />
                </button>
              </div>
              <div className="bg-slate-100 dark:bg-slate-950 p-2 rounded-xl text-right font-mono text-lg text-[#1e3a8a] dark:text-blue-400 border border-slate-200 dark:border-slate-800 mb-3 truncate">
                {calcDisplay}
              </div>
              <div className="grid grid-cols-4 gap-1.5 text-xs font-bold">
                {['7','8','9','÷','4','5','6','×','1','2','3','-','C','0','=','+'].map((btn) => (
                  <button
                    ...
                    className="p-2.5 bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 dark:hover:bg-slate-700 text-slate-800 dark:text-slate-200 rounded-lg active:scale-95 cursor-pointer"
                  >
                    {btn}
                  </button>
                ))}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
            ...
            <div className="lg:col-span-3 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl p-5 sm:p-7 shadow-xs space-y-5">
              ...
              <div className="space-y-2.5">
                {(['A', 'B', 'C', 'D'] as const).map((optKey) => {
                  ...
                  const btnStyle = isSelected
                    ? 'bg-blue-50 dark:bg-blue-950/40 border-[#1e3a8a] dark:border-blue-500 text-[#1e3a8a] dark:text-blue-300 font-bold ring-1 ring-[#1e3a8a]'
                  ...
                  return (
                    <button
                      ...
                      className={`w-full text-left p-3.5 sm:p-4 rounded-xl border text-xs sm:text-sm transition-all flex items-center space-x-3 cursor-pointer ${btnStyle}`}
                    >
                      <span className={`w-7 h-7 rounded-lg text-xs font-bold flex items-center justify-center shrink-0 ${
                        isSelected ? 'bg-[#1e3a8a] text-white' : 'bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-400'
                      }`}>
                        {optKey}
                      </span>
                      <span>{optText}</span>
              ...
              <div className="flex flex-wrap items-center justify-between gap-3 pt-6 border-t border-slate-200 dark:border-slate-800">
                <div className="flex items-center space-x-2">
                  <button
                    ...
                    className="px-4 py-2.5 bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 dark:hover:bg-slate-700 disabled:opacity-40 text-slate-700 dark:text-slate-200 text-xs font-semibold rounded-xl border border-slate-200 dark:border-slate-700 transition-colors flex items-center gap-1 cursor-pointer shadow-xs"
                  >
                  ...
                  <button
                    ...
                    className="px-3 py-2.5 bg-slate-100 hover:bg-slate-200 dark:bg-slate-950 dark:hover:bg-slate-800 text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-slate-200 disabled:opacity-30 text-xs rounded-xl border border-slate-200 dark:border-slate-800 cursor-pointer shadow-xs"
                  >
                    Clear Response
                  </button>
                </div>
                ...
                <div className="flex items-center space-x-2">
                  {currentIndex < examQuestions.length - 1 ? (
                    <button
                      ...
                      className="px-5 py-2.5 bg-[#1e3a8a] hover:bg-[#172554] text-white text-xs font-bold rounded-xl shadow-xs transition-colors flex items-center gap-1 cursor-pointer"
                    >
                      Next
                    ...
                    <button
                      ...
                      className="px-6 py-2.5 bg-[#1e3a8a] hover:bg-[#172554] text-white text-xs font-bold rounded-xl shadow-xs transition-colors cursor-pointer"
                    >
                      Submit Exam
                    </button>
                  )}
            ...
            <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl p-4 sm:p-5 shadow-xs space-y-4">
              ...
              <div className="grid grid-cols-2 gap-2 text-[10px] text-slate-600 dark:text-slate-400">
                <div className="flex items-center gap-1.5">
                  <div className="w-2.5 h-2.5 rounded bg-[#1e3a8a]"></div>
                  <span>Answered</span>
                </div>
                <div className="flex items-center gap-1.5">
                ...
                <div className="flex items-center gap-1.5">
                  <div className="w-2.5 h-2.5 rounded bg-slate-200 dark:bg-slate-800"></div>
                  <span>Unanswered</span>
                </div>
                <div className="flex items-center gap-1.5">
                  <div className="w-2.5 h-2.5 rounded border-2 border-[#1e3a8a]"></div>
                  <span>Current</span>
                </div>
              </div>
              ...
              <div className="grid grid-cols-5 gap-2 max-h-64 overflow-y-auto p-1">
                {examQuestions.map((q, idx) => {
                  ...
                  let style = 'bg-slate-100 dark:bg-slate-950 text-slate-700 dark:text-slate-400 border-slate-200 dark:border-slate-800';

                  if (isAns) {
                    style = 'bg-[#1e3a8a] text-white font-bold border-[#1e3a8a]';
                  } else if (isMarked) {
                    style = 'bg-amber-500 text-white font-bold border-amber-400';
                  }

                  if (isCurr) {
                    style += ' ring-2 ring-[#1e3a8a] ring-offset-1 dark:ring-offset-slate-900';
                  }
              ...
              <button
                onClick={() => setConfirmSubmitOpen(true)}
                className="w-full py-2.5 bg-blue-50 hover:bg-blue-100 dark:bg-blue-950/30 dark:hover:bg-blue-900/40 text-[#1e3a8a] dark:text-blue-300 font-bold text-xs rounded-xl border border-blue-200 dark:border-blue-500/40 transition-colors mt-4 cursor-pointer"
      ...
      {step === 'active' && (!examQuestions[currentIndex] || examQuestions.length === 0) && (
        <div className="bg-slate-900 border border-slate-800 rounded-3xl p-8 text-center max-w-lg mx-auto space-y-4 shadow-xl">
          ...
          <button
            ...
            className="px-6 py-3 bg-[#1e3a8a] hover:bg-[#172554] text-white font-bold text-xs rounded-xl shadow-lg transition-all cursor-pointer"
          >
            Back to Exam Setup
          </button>
        </div>
      ...
      {/* Unlimited Questions Modal */}
      {showUnlimitedPremiumModal && (
        <div className="fixed inset-0 z-50 bg-slate-950/60 backdrop-blur-xs flex items-center justify-center p-4 animate-in fade-in" id="cbt-unlimited-questions-modal">
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-5 sm:p-7 max-w-md w-full shadow-xl relative text-left space-y-4">
            <div className="flex items-center justify-between pb-3 border-b border-slate-200 dark:border-slate-800">
              <button
                onClick={() => {
              ...
              <div className="flex items-center gap-1.5">
                ...
                <span className="text-xs font-bold text-amber-600 dark:text-amber-400 uppercase tracking-wider">Premium Feature</span>
              </div>
            ...
            <p className="text-slate-700 dark:text-slate-300 text-xs font-medium leading-relaxed bg-amber-50 dark:bg-amber-950/30 border border-amber-200 dark:border-amber-500/20 p-3 rounded-xl">
            ...
            <div className="flex items-center justify-end gap-2.5 pt-3 border-t border-slate-200 dark:border-slate-800">
              <button
                ...
                className="px-4 py-2 bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-300 font-semibold text-xs rounded-xl border border-slate-200 dark:border-slate-700 transition-colors cursor-pointer shadow-xs"
              >
                Cancel
              </button>
              <button
                ...
                className="px-4 py-2 bg-[#1e3a8a] hover:bg-[#172554] text-white font-bold text-xs rounded-xl shadow-xs transition-colors flex items-center gap-1.5 cursor-pointer"
              >
      ...
      {confirmSubmitOpen && (
        <div className="fixed inset-0 z-50 bg-slate-950/60 backdrop-blur-xs flex items-center justify-center p-4">
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-5 sm:p-7 max-w-md w-full shadow-xl space-y-4 text-center">
            ...
            <div className="w-12 h-12 rounded-xl bg-amber-50 dark:bg-amber-950/30 text-amber-600 dark:text-amber-400 border border-amber-200 dark:border-amber-500/30 flex items-center justify-center mx-auto">
              <AlertTriangle className="w-6 h-6" />
            </div>
            <h3 className="text-lg font-bold text-slate-900 dark:text-white">Confirm Exam Submission</h3>
            <p className="text-xs text-slate-600 dark:text-slate-300 leading-relaxed">
              You have answered <span className="font-bold text-[#1e3a8a] dark:text-blue-400">{Object.keys(userAnswers).length}</span> out of <span className="font-bold">{examQuestions.length}</span> questions. Are you sure you want to finalize your exam?
            </p>
            <div className="flex gap-2.5 pt-2">
              <button
                onClick={() => setConfirmSubmitOpen(false)}
                className="flex-1 py-2.5 bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-300 font-semibold text-xs rounded-xl border border-slate-200 dark:border-slate-700 transition-colors cursor-pointer shadow-xs"
              >
                Return to Exam
              </button>
              <button
                onClick={handleSubmitExam}
                className="flex-1 py-2.5 bg-[#1e3a8a] hover:bg-[#172554] text-white font-bold text-xs rounded-xl shadow-xs transition-colors cursor-pointer"
              >
                Yes, Submit Now
              </button>
            </div>
