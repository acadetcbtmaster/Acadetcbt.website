...
import { PreJambDatabaseService } from '../../services/prejambDatabaseService';
...
export const PreJambAcademyApp: React.FC<PreJambAcademyAppProps> = ({
  ...
  const [_dbVersion, setDbVersion] = useState<number>(0);
  ...
  // Automatically synchronize latest Pre-JAMB database updates and listen for admin live changes
  useEffect(() => {
    PreJambDatabaseService.syncFromPreJambSupabase().catch(() => {});

    const handleDbUpdate = () => {
      setDbVersion((v) => v + 1);
    };

    window.addEventListener('prejamb_database_updated', handleDbUpdate);
    window.addEventListener('cbt_storage_change', handleDbUpdate);

    return () => {
      window.removeEventListener('prejamb_database_updated', handleDbUpdate);
      window.removeEventListener('cbt_storage_change', handleDbUpdate);
    };
  }, []);
