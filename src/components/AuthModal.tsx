...
export const AuthModal: React.FC<AuthModalProps> = ({
  ...
  // Synchronize mode when initialMode prop changes
  useEffect(() => {
    setMode(initialMode);
  }, [initialMode]);
  ...
  return (
    <div
      className={
        isRegister
          ? "fixed inset-0 z-50 bg-slate-950 overflow-y-auto flex flex-col animate-in fade-in"
          : "fixed inset-0 z-50 bg-slate-950/85 backdrop-blur-md flex items-center justify-center p-3 sm:p-4 animate-in fade-in overflow-y-auto"
      }
    ...
    >
      <div
        className={
          isRegister
            ? "w-full min-h-screen bg-slate-900 flex flex-col relative"
            : "bg-slate-900 border border-slate-800 w-full max-w-lg rounded-2xl shadow-2xl p-5 sm:p-7 relative overflow-hidden max-h-[92vh] flex flex-col"
        }
        id="auth-modal-box"
      >
        ...
        <div
          className={
            isRegister
              ? "sticky top-0 z-40 bg-slate-950/95 backdrop-blur-md px-4 sm:px-8 lg:px-12 py-3 border-b border-slate-800 flex items-center justify-between shrink-0 shadow-sm"
              : "flex items-center justify-between pb-3 border-b border-slate-800 shrink-0 mb-4 z-10"
          }
          id="auth-modal-top-bar"
        >
          {/* Top Left Back Arrow Button & Portal Info */}
          <div className="flex items-center gap-3">
            <button
              onClick={onClose}
              className="p-2 text-slate-300 hover:text-white rounded-xl bg-slate-900 hover:bg-slate-800 transition-colors flex items-center gap-1.5 text-xs font-bold border border-slate-800 cursor-pointer shadow-sm"
            ...
            {isRegister ? (
              <div className="flex items-center gap-2.5">
                <div className="w-7 h-7 rounded-lg bg-indigo-600/20 border border-indigo-500/30 overflow-hidden shrink-0">
                  <img 
                    src={brandLogo} 
                    alt="Acadet CBT Logo" 
                ...
                <div className="flex items-center gap-2">
                  <span className="font-bold text-white text-xs sm:text-sm tracking-tight">Acamet CBT</span>
              ...
              <span className="text-[11px] sm:text-xs font-bold text-slate-300 uppercase tracking-wider">
                {mode === 'login' ? 'Sign In' : mode === 'admin' ? 'Admin Access' : 'Account Recovery'}
              </span>
            )}
          </div>

          {/* Top Center Mode Switcher Tabs for Full-Screen Sign Up */}
          {isRegister && (
            <div className="hidden md:flex items-center p-1 bg-slate-900 rounded-xl border border-slate-800" id="top-bar-mode-switcher">
              <button
                type="button"
                className="px-3.5 py-1.5 text-xs font-bold rounded-lg bg-indigo-600 text-white shadow-sm"
              >
                Sign Up
              </button>
              <button
                ...
                className="px-3.5 py-1.5 text-xs font-medium text-slate-400 hover:text-white rounded-lg transition-colors cursor-pointer"
              >
                Student Sign In
              </button>
              <button
                ...
                className="px-3.5 py-1.5 text-xs font-medium text-slate-400 hover:text-amber-300 rounded-lg transition-colors cursor-pointer"
              >
                Admin
              </button>
            </div>
          ...
          <div className="flex items-center gap-2.5">
            {isRegister && (
              <button
                ...
                className="hidden sm:flex items-center gap-1 text-xs text-slate-400 hover:text-indigo-400 font-medium px-2 py-1 transition-colors cursor-pointer"
              >
                <span>Existing student?</span>
                <span className="text-indigo-400 font-semibold underline">Sign In</span>
              </button>
            )}

            <button
              onClick={onClose}
              className="p-2 text-slate-300 hover:text-white rounded-xl bg-slate-900 hover:bg-slate-800 transition-colors flex items-center gap-1 text-xs font-bold border border-slate-800 cursor-pointer shadow-sm"
        ...
        <div
          className={
            isRegister
              ? "flex-1 w-full max-w-6xl xl:max-w-7xl 2xl:max-w-[1440px] mx-auto px-4 sm:px-6 lg:px-8 xl:px-12 py-4 sm:py-6 flex flex-col justify-start"
        ...
        >
          ...
          {/* Header */}
          {isRegister ? (
            /* Streamlined Screen-Fitting Registration Header */
            <div className="mb-4 sm:mb-6 border-b border-slate-800/80 pb-3.5">
              <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
                <div>
                  ...
                  <p className="text-xs text-slate-400 mt-1 max-w-2xl leading-relaxed">
                    Select your university and enter your credentials to access verified Nigerian CBT questions, mock tests, and smart study guides.
                  </p>
                </div>
                <div className="flex items-center gap-2 shrink-0">
                  <span className="text-[11px] font-semibold text-blue-400 bg-blue-500/10 border border-blue-500/20 px-3 py-1 rounded-full flex items-center gap-1.5">
                    <Sparkles className="w-3.5 h-3.5 text-blue-400" />
                    <span>Includes 30 Free Practice Questions</span>
                  </span>
                </div>
              </div>
            </div>
          ) : (
            /* Header for Login / Admin / Forgot Modes */
            <div className="text-center mb-5 pt-1">
              ...
              <h1 className="text-2xl sm:text-3xl font-bold text-white tracking-tight">
                ...
                {mode === 'guest' && 'Guest Practice Access'}
              </h1>
              <p className="text-xs sm:text-sm text-slate-400 mt-1.5 max-w-lg mx-auto leading-relaxed">
                ...
                {mode === 'guest' && 'Enjoy instant practice access with 30 free questions without signing in.'}
              </p>
            </div>
          )}

          {/* Mode Selector Tabs (only shown when not in full-screen register and not in forgot) */}
          {!isRegister && mode !== 'forgot' && (
            <div className="grid grid-cols-3 p-1 bg-slate-950 rounded-xl border border-slate-800 mb-6 max-w-md mx-auto w-full" id="mode-selector-tab">
              <button
                type="button"
                onClick={() => {
                  setMode('register');
                ...
                className="py-2 text-[11px] font-semibold rounded-lg transition-all flex items-center justify-center gap-1 cursor-pointer text-slate-400 hover:text-slate-200"
                id="tab-register"
              >
          ...
          {topBannerSuccess && (
            <div className="mb-4 bg-blue-500/10 border border-blue-500/30 text-blue-300 rounded-xl p-3 text-xs flex items-center gap-2.5 animate-in fade-in" id="auth-top-banner-success">
          ...
          {mode === 'forgot' ? (
            <div className="space-y-4">
              ...
              {recoverySuccess && (
                <div className="bg-blue-500/10 border border-blue-500/30 text-blue-300 rounded-xl p-3 text-xs flex items-center gap-2.5 animate-in fade-in" id="auth-recovery-success">
                  <CheckCircle2 className="w-4 h-4 text-blue-400 shrink-0" />
              ...
              {forgotStep === 'verify' ? (
                ...
                <form onSubmit={handlePasswordChange} className="space-y-4">
                  ...
                  <div className="bg-blue-500/10 border border-blue-500/30 rounded-2xl p-3.5 text-blue-300 text-xs flex items-center gap-3">
                    <div className="w-8 h-8 rounded-full bg-blue-500/20 flex items-center justify-center shrink-0 border border-blue-500/40">
            ...
            <form onSubmit={handleGuestSubmit} className="space-y-3.5" noValidate>
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    className="w-full pl-9 pr-3 py-2.5 bg-slate-950 border border-slate-800 focus:border-blue-500 rounded-xl text-xs text-slate-100 focus:outline-none transition-all"
                  />
                </div>
              </div>
              ...
              <button
                type="submit"
                disabled={isSubmitting}
                className="w-full py-3.5 bg-blue-600 hover:bg-blue-500 disabled:opacity-50 text-white font-bold text-xs rounded-xl shadow-xl shadow-blue-600/25 transition-all flex items-center justify-center gap-2 cursor-pointer active:scale-[0.99]"
                id="guest-submit-btn"
              >
                {isSubmitting ? (
                  <>
            ...
            <form onSubmit={handleRegister} className="space-y-6" noValidate>
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-5 xl:gap-6 items-start">
                ...
                {/* Column 1: Account & Security Credentials */}
                <div className="bg-slate-900/80 border border-slate-800/90 rounded-2xl p-4 sm:p-5 space-y-3.5 shadow-sm">
            ...
            {/* Column 2: Academic Profile & Submission */}
            <div className="bg-slate-900/80 border border-slate-800/90 rounded-2xl p-4 sm:p-5 space-y-3.5 shadow-sm flex flex-col justify-between">
              <div className="space-y-3.5">
                <div className="flex items-center gap-2 pb-2.5 border-b border-slate-800">
                  <GraduationCap className="w-4 h-4 text-indigo-400 shrink-0" />
                  <h2 className="text-xs sm:text-sm font-bold text-white uppercase tracking-wider">
                    2. Academic Profile & Verification
                  </h2>
                </div>
              ...
              <div ref={termsContainerRef} className="space-y-2 pt-1 border-t border-slate-800/80">
                ...
                <div className="pt-2.5 border-t border-slate-800/80 space-y-2.5">
                  ...
                  <button
                    type="submit"
                    disabled={isSubmitting}
                    className="w-full py-3 bg-indigo-600 hover:bg-indigo-500 disabled:opacity-50 text-white font-bold text-xs sm:text-sm rounded-xl shadow-xl shadow-indigo-600/25 transition-all flex items-center justify-center gap-2 cursor-pointer active:scale-[0.99]"
                  >
                    {isSubmitting ? (
                      <>
                        <Loader2 className="w-4 h-4 animate-spin text-white" />
                  ...
                  <button
                    ...
                    className="w-full py-2.5 bg-blue-500/10 hover:bg-blue-500/20 border border-blue-500/30 hover:border-blue-500/50 text-xs font-bold text-blue-300 rounded-xl flex items-center justify-center gap-2 transition-all cursor-pointer shadow-sm active:scale-[0.99]"
        ...
        {/* Alternative Quick Access: Guest Mode (Shown for non-register modes) */}
        {mode !== 'forgot' && mode !== 'admin' && mode !== 'guest' && !isRegister && (
          <div className="mt-6 border-t border-slate-800/80 pt-4 text-center space-y-2.5 max-w-md mx-auto w-full">
            <p className="text-[11px] font-semibold text-slate-500 mb-2 tracking-wider">OR QUICK ACCESS</p>
        ...
        {/* Bottom Switch Text (Shown for non-register modes) */}
        {!isRegister && (
          <div className="mt-4 text-center text-xs text-slate-400 max-w-md mx-auto w-full pb-4">
            {mode === 'login' ? (
              <p>
