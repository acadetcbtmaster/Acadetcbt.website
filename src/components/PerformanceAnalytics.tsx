...
export const PerformanceAnalytics: React.FC<PerformanceAnalyticsProps> = ({
  ...
  return (
    <div className="w-full max-w-6xl mx-auto px-4 py-8 space-y-6" id="analytics-container">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-2xl p-5">
          ...
          <p className="text-2xl font-bold text-blue-400">{mockExams.length}</p>
          ...
          <div className="space-y-3">
            {[
              ...
              <div key={idx} className="p-3 bg-slate-950 rounded-xl border border-slate-800 space-y-1.5 text-xs">
                <div className="flex justify-between items-center">
                  <span className="font-semibold text-slate-200">{t.topic} ({t.course})</span>
                  <span className="text-blue-400 font-bold text-xs">{t.accuracy}% Accuracy</span>
                </div>
                <div className="w-full h-1.5 bg-slate-800 rounded-full overflow-hidden">
                  <div className="h-full bg-blue-500" style={{ width: `${t.accuracy}%` }}></div>
                </div>
              </div>
            ))}
          </div>
                  ...
                  <tr key={r.id} className="hover:bg-slate-800/40">
                    ...
                    <td className="p-3">
                      <span className={`px-2 py-0.5 rounded text-[10px] font-bold uppercase ${
                        r.type === 'mock_cbt' ? 'bg-blue-500/20 text-blue-300' : 'bg-indigo-500/20 text-indigo-300'
                      }`}>
                        {r.type === 'mock_cbt' ? 'Mock CBT' : 'Practice'}
                      </span>
                    </td>
                    <td className="p-3">{r.score}/{r.totalQuestions}</td>
                    <td className="p-3 font-bold text-blue-400">{r.percentage}%</td>
