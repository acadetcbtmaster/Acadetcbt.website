...
export const LeaderboardView: React.FC<LeaderboardViewProps> = ({
  ...
  return (
    <div className="w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 space-y-5" id="leaderboard-view">
        ...
        <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            <div className="flex items-center gap-2 mb-2">
              <span className="px-2.5 py-0.5 rounded-full text-[11px] font-bold uppercase tracking-wider bg-blue-50 dark:bg-blue-500/10 text-[#1e3a8a] dark:text-blue-400 border border-blue-200 dark:border-blue-500/20 flex items-center gap-1.5">
                <Trophy className="w-3.5 h-3.5 text-[#1e3a8a] dark:text-blue-400" />
            ...
            <h1 className="text-xl sm:text-2xl font-bold text-slate-900 dark:text-white">Student Academic Rankings</h1>
          ...
          {!isPremium && (
            <button
              ...
              className="px-4 py-2.5 bg-[#1e3a8a] hover:bg-[#172554] text-white font-bold text-xs rounded-xl shadow-xs transition-colors flex items-center gap-2 shrink-0 cursor-pointer"
      ...
      <div className="space-y-5 animate-in fade-in">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4" id="leaderboard-podium">
          {topStudents.slice(0, 3).map((st) => (
            <div
              key={st.id}
              className={`bg-white dark:bg-slate-900 border rounded-xl p-5 relative overflow-hidden shadow-xs flex flex-col justify-between ${
                st.rank === 1
                  ? 'border-amber-400 dark:border-amber-500/40'
                  : st.rank === 2
                  ? 'border-slate-300 dark:border-slate-700'
                  : 'border-amber-600/30 dark:border-amber-700/40'
              }`}
            >
              <div className="flex items-center justify-between mb-3">
                <span
                  className={`text-xl font-black px-2.5 py-0.5 rounded-lg border ${
                    st.rank === 1
                ...
                <span className="text-[11px] font-bold text-amber-700 dark:text-amber-300 bg-amber-50 dark:bg-amber-500/10 px-2 py-0.5 rounded-full border border-amber-200 dark:border-amber-500/20 flex items-center gap-1">
              ...
              <div>
                ...
                <p className="text-[11px] text-slate-500 dark:text-slate-400 mt-0.5">{st.departmentName}</p>
                <p className="text-[10px] text-[#1e3a8a] dark:text-blue-400 font-semibold">{st.universityName}</p>
              </div>
              ...
              <div className="mt-4 pt-3 border-t border-slate-100 dark:border-slate-800 grid grid-cols-3 gap-2 text-center text-xs">
                <div>
                  ...
                  <p className="font-bold text-[#1e3a8a] dark:text-blue-400">{st.scoreAvg}%</p>
                </div>
                <div>
                  <p className="text-[10px] text-slate-400 dark:text-slate-500">Questions</p>
                  <p className="font-bold text-slate-800 dark:text-slate-200">{st.totalQuestionsAttempted}</p>
                </div>
                <div>
        ...
        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl p-5 space-y-4 shadow-xs">
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left text-xs text-slate-700 dark:text-slate-300">
              <thead className="text-[11px] uppercase bg-slate-50 dark:bg-slate-950 text-slate-500 dark:text-slate-400 border-b border-slate-200 dark:border-slate-800">
                <tr>
                  ...
                  return (
                    <tr
                      ...
                      className={`transition-colors ${
                        isCurrentUser
                          ? 'bg-blue-50/60 dark:bg-blue-950/30 border-l-4 border-[#1e3a8a] dark:border-blue-500 font-bold'
                          : 'hover:bg-slate-50 dark:hover:bg-slate-800/40'
                      }`}
                    >
                      <td className="p-3 font-bold text-slate-800 dark:text-slate-200">#{st.rank}</td>
                      <td className="p-3 font-bold text-slate-900 dark:text-white flex items-center gap-2">
                        {st.name}
                        {isCurrentUser && (
                          <span className="bg-blue-100 dark:bg-blue-500/20 text-[#1e3a8a] dark:text-blue-300 border border-blue-300 dark:border-blue-500/30 text-[9px] px-1.5 py-0.5 rounded uppercase">
                            You
                          </span>
                        )}
                      </td>
                      <td className="p-3">
                        <p className="text-slate-800 dark:text-slate-200 font-medium">{st.departmentName}</p>
                        <p className="text-[10px] text-slate-500">{st.universityName}</p>
                      </td>
                      <td className="p-3 font-bold text-[#1e3a8a] dark:text-blue-400">{st.scoreAvg}%</td>
                      <td className="p-3 font-semibold text-slate-800 dark:text-slate-200">{st.totalQuestionsAttempted}</td>
                      <td className="p-3 font-bold text-amber-600 dark:text-amber-400 flex items-center gap-1">
                      ...
                      <td className="p-3">
                        <span
                          className={`px-2 py-0.5 rounded text-[10px] font-bold ${
                            st.isPremium
                              ? 'bg-blue-100 dark:bg-blue-500/10 text-[#1e3a8a] dark:text-blue-300 border border-blue-300 dark:border-blue-500/20'
