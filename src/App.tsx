...
export default function App() {
  ...
  // Day / Night Theme Mode State (Defaults to 'light' for all first-time visitors)
  ...
  const [activeTab, setActiveTab] = useState<string>(() => {
    if (window.location.pathname.includes('/founder') || window.location.hash === '#founder') {
      return 'founder';
    }
  ...
  return (
    <div
      className={`min-h-screen w-full max-w-full overflow-x-hidden flex flex-col font-sans selection:bg-indigo-500 selection:text-white ${
        themeMode === 'light' ? 'bg-white text-slate-900' : 'bg-slate-950 text-slate-100'
      }`}
      id="cbt-app-root"
    >
      ...
      {/* Main View Router */}
      <main className="flex-1 w-full max-w-full py-6 pb-24 md:pb-6" id="main-content">
      ...
      {/* Footer */}
      <footer
        className="border-t border-slate-200 dark:border-slate-800/80 bg-white dark:bg-slate-950/60 pt-8 pb-32 sm:pb-36 md:pb-8 text-xs text-slate-600 dark:text-slate-400 w-full"
        id="global-cbt-footer"
      >
        <div className="w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex flex-col sm:flex-row items-center justify-between gap-4">
          ...
          <div className="flex flex-wrap items-center gap-4 text-slate-600 dark:text-slate-400">
            <button
              ...
              className="text-amber-600 dark:text-amber-400 hover:text-amber-700 dark:hover:text-amber-300 font-bold cursor-pointer flex items-center gap-1"
            ...
            <button
              ...
              className="text-[#1e3a8a] dark:text-blue-400 hover:underline font-semibold cursor-pointer"
            >
              About Acadet
            </button>
            <a
            ...
            <button onClick={() => setSubModalOpen(true)} className="hover:text-slate-900 dark:hover:text-white cursor-pointer">Subscriptions</button>
            {!currentUser && (
              <a href="#faq" onClick={() => handleNavigate('landing')} className="hover:text-slate-900 dark:hover:text-white cursor-pointer">FAQ</a>
            )}
          </div>
        </div>
      </footer>
      ...
      {registrationMessage && (
        <div className="fixed inset-0 z-50 bg-slate-950/60 backdrop-blur-sm flex items-center justify-center p-4 animate-in fade-in" id="public-notification-modal">
          <div className="bg-white dark:bg-slate-900 border border-blue-500/40 max-w-md w-full rounded-2xl p-6 shadow-xl relative text-left flex flex-col space-y-4">
            
            {/* Top Navigation Bar with Back & Cancel Buttons */}
            <div className="flex items-center justify-between pb-3 border-b border-slate-100 dark:border-slate-800">
              <button
                onClick={() => setRegistrationMessage(null)}
                className="p-2 text-slate-700 dark:text-slate-300 hover:text-slate-900 dark:hover:text-white rounded-lg bg-slate-100 dark:bg-slate-800 transition-colors flex items-center gap-1.5 text-xs font-bold border border-slate-200 dark:border-slate-700 cursor-pointer"
              ...
              <button
                onClick={() => setRegistrationMessage(null)}
                className="p-2 text-slate-700 dark:text-slate-300 hover:text-slate-900 dark:hover:text-white rounded-lg bg-slate-100 dark:bg-slate-800 transition-colors flex items-center gap-1 text-xs font-bold border border-slate-200 dark:border-slate-700 cursor-pointer"
            ...
            <div className="flex items-start gap-3 pt-2">
              <div className="w-10 h-10 bg-blue-50 text-[#1e3a8a] dark:bg-blue-500/20 dark:text-blue-400 border border-blue-200 dark:border-blue-500/40 rounded-xl flex items-center justify-center shrink-0">
                <CheckCircle2 className="w-5 h-5 text-[#1e3a8a] dark:text-blue-400" />
              </div>
              <div className="space-y-1">
                <h3 className="text-base font-bold text-slate-900 dark:text-white tracking-tight">CBT Master Notice</h3>
                <p className="text-xs text-slate-600 dark:text-slate-200 leading-relaxed">{registrationMessage}</p>
              </div>
            </div>
            ...
            <div className="pt-2">
              <button
                onClick={() => setRegistrationMessage(null)}
                className="w-full py-2.5 bg-[#1e3a8a] hover:bg-[#172554] text-white font-bold text-xs rounded-xl shadow-xs transition-all flex items-center justify-center gap-2 cursor-pointer"
