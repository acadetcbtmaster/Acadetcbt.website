...
export class StorageService {
  ...
  static getQuickLinks(): QuickLinkItem[] {
    ...
    const items = this.getItem<QuickLinkItem[]>(STORAGE_KEYS.QUICK_LINKS, defaultLinks);
    // Ensure scholarship updates are completely removed even if cached
    const cleaned = items.filter(
      (l) => l.id !== 'ql-4' && !l.title?.toLowerCase().includes('scholarship') && !l.description?.toLowerCase().includes('scholarship')
    );
    return cleaned;
  }
  ...
  static getHomepageSections(): HomepageSection[] {
    const defaultSections: HomepageSection[] = [
      ...
      },
      {
        ...
        description: 'Explore live Pre-JAMB Acadet CBT Tests, WhatsApp channels, and verified study materials.',
        status: 'active',
        order: 2,
        createdAt: new Date().toISOString(),
      },
    ...
    const sections = this.getItem<HomepageSection[]>(STORAGE_KEYS.HOMEPAGE_SECTIONS, defaultSections);
    // Ensure sec-3 uses the CBT candidates photo, no scholarship mentions, and no emojis in titles/badges
    const emojiRegex = /[\u{1F300}-\u{1F9FF}\u{2600}-\u{26FF}\u{2700}-\u{27BF}]/gu;
    return sections.map((s) => {
      const cleanTitle = (s.title || '').replace(emojiRegex, '').trim();
      const cleanBadge = (s.badge || '').replace(emojiRegex, '').trim();
      const cleanSubtitle = (s.subtitle || '').replace(emojiRegex, '').trim();

      if (s.id === 'sec-3' || s.title?.includes('Pre-JAMB')) {
        return {
          ...s,
          title: cleanTitle,
          badge: cleanBadge,
      ...
      if (s.id === 'sec-2' && s.description?.includes('scholarship')) {
        return {
          ...s,
          title: cleanTitle,
          badge: cleanBadge,
