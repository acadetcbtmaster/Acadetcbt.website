...
export class PreJambDatabaseService {
  ...
  // =========================================================================
  // SUPABASE ASYNC SYNCHRONIZATION HELPERS & EVENT BROADCASTING
  // =========================================================================

  public static broadcastDatabaseUpdate(): void {
    if (typeof window !== 'undefined') {
      try {
        window.dispatchEvent(
          new CustomEvent('prejamb_database_updated', {
            detail: {
              timestamp: Date.now(),
              totalQuestions: this.questionsCache?.length || 0,
              totalSubjects: this.subjectsCache?.length || 0,
            },
          })
        );
        window.dispatchEvent(new CustomEvent('cbt_storage_change', { detail: { key: 'prejamb_questions' } }));
      } catch {}
    }
  }
  ...
  public static async syncFromPreJambSupabase(): Promise<boolean> {
    ...
    // 1. Primary fast-path: Query server-side proxy catalog endpoint
    try {
      const resp = await fetch('/api/prejamb/catalog');
      if (resp.ok) {
        ...
        if (data && data.success) {
          if (Array.isArray(data.subjects) && data.subjects.length > 0) {
            ...
            localStorage.setItem('prejamb_db_subjects_v1', safeStringify(this.subjectsCache));
            syncedAny = true;
          }
          if (Array.isArray(data.questions) && data.questions.length > 0) {
            ...
            localStorage.setItem('prejamb_db_questions_v1', safeStringify(this.questionsCache));
            syncedAny = true;
          }
          ...
          if (Array.isArray(data.announcements) && data.announcements.length > 0) {
            this.announcementsCache = data.announcements;
            localStorage.setItem(STORAGE_KEYS.ANNOUNCEMENTS, safeStringify(this.announcementsCache));
            syncedAny = true;
          }
          if (data.settings) {
            this.settingsCache = {
              id: data.settings.id || 'prejamb_system_config',
              systemName: data.settings.system_name || 'Pre-JAMB Academy CBT Platform',
              enableInstantResults: data.settings.enable_instant_results ?? true,
              enableAnswerReview: data.settings.enable_answer_review ?? true,
              enableKeyboardShortcuts: data.settings.enable_keyboard_shortcuts ?? true,
              mockExamDurationMinutes: data.settings.mock_exam_duration_minutes || 120,
              defaultPassPercentage: data.settings.default_pass_percentage || 50,
              announcementBanner: data.settings.announcement_banner || '',
              updatedAt: data.settings.updated_at || new Date().toISOString(),
            };
            localStorage.setItem(STORAGE_KEYS.SETTINGS, safeStringify(this.settingsCache));
            localStorage.setItem('prejamb_db_config_v1', safeStringify(this.settingsCache));
          }
    ...
    // 2. Direct client fallback query to Supabase
    const client = getPreJambSupabaseClient();
    if (!client) return syncedAny;
    ...
    try {
      ...
      if (!cErr && candData && candData.length > 0) {
        ...
        localStorage.setItem(STORAGE_KEYS.CANDIDATES, safeStringify(this.candidatesCache));
        localStorage.setItem('prejamb_db_candidates_v1', safeStringify(this.candidatesCache));
        syncedAny = true;
      }
  ...
  private static async syncDocToPreJambSupabase(table: string, payload: any): Promise<void> {
    ...
    // 2. Server API fallback sync for cross-network reliability
    try {
      const typeKey = table.replace('prejamb_', '');
      await fetch('/api/prejamb/sync', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ [typeKey]: [payload] }),
      });
    } catch {}
  ...
  private static async deleteDocFromPreJambSupabase(table: string, id: string): Promise<void> {
    ...
    try {
      const typeKey = table.replace('prejamb_', '');
      await fetch(`/api/prejamb/${typeKey}/${id}`, {
        method: 'DELETE',
      });
    } catch {}
  ...
  public static bulkSavePreJambQuestions(questions: PreJambQuestion[]): void {
    ...
    // Batch upsert to Pre-JAMB Supabase via API & direct client
    try {
      fetch('/api/prejamb/sync', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ questions: Array.from(map.values()) }),
      }).catch(() => {});
    } catch {}
